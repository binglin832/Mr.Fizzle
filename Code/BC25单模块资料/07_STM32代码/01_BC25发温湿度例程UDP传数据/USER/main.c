...
//适用于NB版本
 ...
 {	
   ...
   ///  while(DHT11_Init())//有温湿度传感器，将此处屏蔽打开
    {}
    OPEN_BC25();//对BC25开机
    BC25_Init();//对设备初始化
    BC25_CreateUDPSokcet();//创建一个SOCKET连接
   while(1)
    {  
    //  DHT11_Read_Data(&temp,&humi);//读取温湿度数据，有温湿度传感器，将此处屏蔽打开
			temp=23;humi=67;//如果你有温湿度传感器，注意将这个地方屏蔽掉
