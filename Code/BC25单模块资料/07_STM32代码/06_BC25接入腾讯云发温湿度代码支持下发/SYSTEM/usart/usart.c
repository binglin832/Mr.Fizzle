...
char RxBuffer[500];     //接收缓冲,最大USART_REC_LEN个字节.
int  RxCounter; 
...
void uart2_init(u32 bound)
{
  ...
  USART_Init(USART2, &USART_InitStructure); //初始化串口
    ...
    USART_ITConfig(USART2, USART_IT_RXNE, ENABLE);//开启中断
		USART_ITConfig(USART2, USART_IT_IDLE, ENABLE);//开启中断idle
...
void Uart2_SendDATA(char*SendBuf,char len)//串口2打印数据
{
  u8 i;
	for(i=0;i<len;i++)
	{
...
void USART2_IRQHandler(void)                	//串口2中断服务程序
    ...
    if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)  //接收模块返回的数据
		{    
 		...
 		if(USART_GetITStatus(USART2, USART_IT_IDLE) != RESET)  //接收模块返回的数据
		{    
           Res =USART_ReceiveData(USART2);//接收模块的数据
           uart2_getok=1;
     } 
